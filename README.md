...
To install the dependencies, run the following command: `pip install -r requirements.txt`. After the installation is done,
...
The GUI is divided into two sections: the services section and the algorithms section. Each section has a set of checkboxes and buttons.
...
The `Edit Services/Algorithms` button allows you to add or remove algorithms from a service/algorithm type category. The `Acquire Data`
