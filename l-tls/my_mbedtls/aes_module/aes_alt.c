...
int mbedtls_aes_crypt_ecb(mbedtls_aes_context *ctx, int mode, const unsigned char input[16], unsigned char output[16]) {
	if(mode == MBEDTLS_AES_ENCRYPT) {
		aes_encrypt(input, output, ctx->key_schedule, ctx->keysize);
	} else if(mode == MBEDTLS_AES_DECRYPT) {
		aes_decrypt(input, output, ctx->key_schedule, ctx->keysize);
	}
...
int mbedtls_aes_crypt_cbc(mbedtls_aes_context *ctx, int mode, size_t length, unsigned char iv[16], const unsigned char *input, unsigned char *output) {
	...
	if(mode == MBEDTLS_AES_ENCRYPT) {
		ret = aes_encrypt_cbc(input, length, output, ctx->key_schedule, ctx->keysize, iv);
	} else if(mode == MBEDTLS_AES_DECRYPT) {
		ret = aes_decrypt_cbc(input, length, output, ctx->key_schedule, ctx->keysize, iv);
	}
