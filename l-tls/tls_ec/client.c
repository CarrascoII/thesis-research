...
int main(int argc, char **argv) {
    ...
    if((ret = mbedtls_x509_crt_parse(&cli_cert, (const unsigned char *) mbedtls_test_cli_crt_ec, mbedtls_test_cli_crt_ec_len)) != 0) {
#if defined(MBEDTLS_DEBUG_C)
        printf(" failed! mbedtls_x509_crt_parse returned -0x%04x\n", -ret);
#endif
        goto exit;
    ...
    if((ret = mbedtls_pk_parse_key(&cli_key, (const unsigned char *) mbedtls_test_cli_key_ec, mbedtls_test_cli_key_ec_len, NULL, 0)) != 0) {
#if defined(MBEDTLS_DEBUG_C)
