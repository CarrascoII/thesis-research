...
def main(argv):
    try:
        ...
        print('One of the options does not exit.\nUse: "algs_profiler.py -h" for help')
        sys.exit(2)
    ...
    for opt, arg in opts:
        if opt in ('-h', '--help'):
            print('algs_profiler.py [-t <compilation_target>] [-w <filter_weight>] ' +
                '[-s <initial_lvl>,<final_lvl>] [-i <initial_size>,<final_size>] [-n <n_tests>] ' +
                '[-d <data_directory>] [-c] [-m] [-k] <services_list>')
            print('algs_profiler.py [--target=<compilation_target>] [--weight=<filter_weight>] ' +
